...
const TRACKS = [
  { n: "01", title: "intro", file: "01-intro", dur: "2:08", accent: "#F2D9A0", meta: "2:08 · 120 BPM", tileMeta: "2:08 · 120 BPM",
    ...
    tile: [0, 0, 2, 1] },
  { n: "02", title: "vieux messages", file: "02-vieux-messages", dur: "1:43", accent: "#C8622A", meta: "1:43 · 132 BPM", tileMeta: "1:43 · 132 BPM",
    ...
    tile: [2, 0, 1, 1] },
  { n: "03", title: "sidehustle", file: "03-sidehustle", dur: "2:54", accent: "#F2E84D", meta: "2:54 · 75 BPM", tileMeta: "2:54 · 75 BPM",
    ...
    tile: [3, 0, 1, 1] },
  { n: "04", title: "buisness model", file: "04-buisness-model", dur: "2:37", accent: "#7FE0A0", meta: "2:37 · 74 BPM", tileMeta: "2:37 · 74 BPM",
  ...
  { n: "06", title: "imperméable", file: "06-impermeable", dur: "2:46", accent: "#B8652E", meta: "2:46 · 75 BPM", tileMeta: "2:46 · 75 BPM",
    ...
    tile: [3, 2, 1, 2] },
  { n: "07", title: "long fleuve", file: "07-long-fleuve", dur: "2:14", accent: "#E0A33A", meta: "2:14 · 92 BPM", tileMeta: "2:14 · 92 BPM",
    ...
    tile: [2, 3, 1, 1] },
  { n: "08", title: "sésame interlude", file: "08-sesame-interlude", dur: "1:50", accent: "#F2E9D8", meta: "1:50 · 74 BPM", tileMeta: "1:50 · 74 BPM",
    ...
    tile: [0, 3, 2, 1] },
  { n: "09", title: "dequoituparles!", file: "09-dequoituparles", dur: "1:30", accent: "#B8A0D0", meta: "1:30 · 126 BPM", tileMeta: "1:30 · 126 BPM",
    ...
    tile: [0, 2, 1, 1] },
  { n: "10", title: "persephone", file: "10-persephone", dur: "3:15", accent: "#F0B8C8", meta: "3:15 · 110 BPM", tileMeta: "3:15 · 110 BPM",
    ...
    tile.innerHTML = `
      <div class="tile-in">
        <div class="tile-front" style="background-size:${N / w * 100}% ${N / h * 100}%;background-position:${pct(c, w)} ${pct(r, h)}">
          <span class="tile-n">${t.n}</span><span class="tile-title">${esc(t.title)}</span>
        </div>
        <div class="tile-back">
          ...
          <div class="tile-back-txt">
            <div class="tile-meta"><span style="color:${t.accent}">${t.n}</span><span>${esc(t.tileMeta)}</span></div>
...
function renderLivret() {
  const grid = $('livret-grid');
  TRACKS.forEach(t => {
    const a = document.createElement('a'); a.className = 'thumb'; a.href = '#pochette'; a.dataset.thumb = t.n;
    a.style.backgroundImage = `url(${t.img})`;
    a.innerHTML = `<span class="thumb-n">${t.n}</span><span class="thumb-title">${esc(t.title)}</span>`;
    a.addEventListener('click', e => { e.preventDefault(); openPlayer(t.i, a.getBoundingClientRect()); });
    grid.appendChild(a);
  });
}
...
function applyTrack(smooth) {
  ...
  const setText = () => { $('p-title').textContent = t.title; meta.textContent = t.meta; $('p-dur').textContent = t.dur; renderLyrics(); renderProgress(); };
...
function setPlaying(on) {
  ...
  if (on) {
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => {
      if (!P.liveOn) return;
      P.playing = true; pLive.classList.add('is-on'); btn.textContent = 'PAUSE'; audioPlay();
    }));
  } else {
...
/* ---------- init ---------- */
renderPuzzle(); renderTracklist(); renderLivret();
const cover = $('titres-cover');
cover.style.backgroundImage = `url(${TRACKS[0].img})`;
cover.addEventListener('click', e => { e.preventDefault(); openPlayer(0, rectOf('01', cover)); });
...
$('p-seek').addEventListener('click', e => {
  ...
  audioSeek(P.elapsed); renderProgress();
});
...
window.addEventListener('resize', () => { if (P.phase === 'open') { setBox(layout().box); placeText(); } });
loadLyrics();
